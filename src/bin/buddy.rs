...
const AUTONOMY_DIRECTIVE: &str = "Autonomy tick: re-evaluate the whole factory from the authoritative snapshot below before acting. The factory is a set of independent subsystems that keep running while you work elsewhere. Choose from the current evidence, not from conversational momentum or the previous turn's focus. Compare the highest-leverage local intervention with the ranked expansion.resource_candidates; do not remain near the current footprint merely because it is familiar. If a required resource or useful expansion direction has no generated candidate, use find_nearest_resource with bounded exploration rather than defaulting to local work. If research or another subsystem is healthy and progressing, leave it running; do not wait for it, repeatedly poll it, or keep embellishing it. Select the highest-leverage stalled or underdeveloped subsystem shown by the current data, inspect the relevant location with tools, take concrete action toward durable automation, and verify the result. Do not merely describe a plan.";
...
mod tests {
    ...
    fn autonomy_prompt_requires_global_reprioritization_and_includes_snapshot() {
        ...
        assert!(prompt.contains("Compare the highest-leverage local intervention"));
        assert!(prompt.contains("ranked expansion.resource_candidates"));
        assert!(prompt.contains("find_nearest_resource with bounded exploration"));
